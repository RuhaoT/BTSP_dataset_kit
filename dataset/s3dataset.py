...
class MinimalBTSPDataset:
    ...
    def __init__(self, coordinate_precision: int = 8, color_precision: int = 8):
        ...
        self.precise_raw_data: pd.DataFrame = None
    ...
    def from_raw_data(self, raw_data: pd.DataFrame):
        """Load the dataset from a raw data table."""
        self.precise_raw_data = raw_data
        return self

    def from_file(self, input_file: str):
        """Load the dataset from a CSV file."""
        self.input_file = input_file
        self.precise_raw_data = pd.read_csv(input_file)

        # check the schema of the input file
        if not self.precise_raw_data.columns.equals(pd.Index(self.file_schema)):
            raise ValueError(
                (
                    "Input file schema is incorrect. Expected:"
                    f"{self.file_schema}"
                    ", got:"
                    f"{self.precise_raw_data.columns}"
                )
            )
    ...
    def from_binary_tensors(self, input_binary_tensors: np.ndarray):
        ...
        input_binary_tensors = input_binary_tensors.T
    ...
    def new_dataset(
        ...
        self.precise_raw_data = pd.DataFrame(
    ...
    def save_dataset(self, output_file: str = None):
        ...
        self.precise_raw_data.to_csv(output_file, index=False)
    ...
    def to_binary_tensors(self, masked: bool = False) -> np.ndarray:
        ...
        for row_index in range(self.precise_raw_data.shape[0]):
...
if __name__ == "__main__":
    ...
    for mask in example.mask_types:
        ...
        example_from_tensors.plot_dataset(save_as=f"dataset_{mask}_test.png")
